...
- [第1章 计算机网络和因特网](#-第1章-计算机网络和因特网-)
  ...
  - [1.4 分组交换网中的时延、丢包和吞吐量](#-14-分组交换网中的时延-丢包和吞吐量-)
...
- [第3章 运输层](#-第3章-运输层-)
  ...
  - [3.6 拥塞控制原理](#-36-拥塞控制原理-)
    ...
    - [3.7.1 公平性](#-371-公平性-)
- [第4章 网络层：数据平面](#-第4章-网络层数据平面-)
  - [4.1 网络层概述](#-41-网络层概述-)
  - [4.2 路由器工作原理](#-42-路由器工作原理-)
  - [4.3 网际协议：IPV4、寻址、IPV6及其他](#-43-网际协议ipv4-寻址-ipv6及其他-)
...
## 3.3 无连接运输：UDP
- 使用UDP的应用可以在应用程序自身中建立可靠性机制，如谷歌chrome的QUIC协议。UDP连接可能在网络中被拦截。
### 3.3.1 UDP报文段结构
- 长度包含首部和数据，单位字节。
...
# 第4章 网络层：数据平面
- 网络层分解为2个相互作用的部分：数据平面和控制平面。
- 数据平面：网络层中每台路由器的功能，从输入链路向输出链路转发数据报：
  1. 传统的IP转发：基于数据报的目的地址。
  2. 通用的转发：基于数据报首部。
- 控制平面：控制数据报沿着从源主机到目的主机的端到端路径中路由器之间的路由方式。路由选择算法（routing algorithm）。


## 4.1 网络层概述
- 转发（forwarding）通常由硬件实现，通常为几纳秒。
  路由选择（routing）通常由软件实现，通常为几秒。
- 转发表（forwarding table）：路由器使用首部的一个或多个字段作为索引，得到输出链路接口。由路由选择算法设置。
- SDN（Software-Defined Networking，SDN）软件定义网络：用软件实现计算转发表并与路由器交互的控制器，远程控制器可能实现在具有高度可靠性和冗余的远程数据中心，并可能由ISP或某些第三方管理。
- 分组交换机：一台通用分组交换设备，根据分组首部字段值，从输入链路接口到输出链路接口转移分组。
  1. 链路层交换机：链路层设备，基于链路层帧的首部。
  2. 路由器：网络层设备，基于网络层数据报的首部。


## 4.2 路由器工作原理
- 路由选择处理器：维护路由选择表、关联链路状态信息、为该路由器计算转发表（在SDN路由器中，负责与远程控制器通信，接收转发表）、网络管理。
![router](pic/router.png)

- 每个输入端口都有转发表的副本，在输入端口处查找转发表，不需要路由选择处理器。
- 最长前缀匹配（longest prefix matching rule）。
- 经内存交换、经总线交换、经互联网络交换等。
![router input port](pic/router%20input%20port.png)
![router output port](pic/router%20output%20port.png)

- 排队：丢弃到达的分组即弃尾（drop-tail）策略、删除一个或多个已排队分组。主动队列管理算法（Active Queue Management,AQM）：随机早期检测算法（Random Early Detection,RED）。
- 分组调度：
  1. 先进先出FIFO，或称为先来先服务FCFS。
  2. 优先权排队：非抢占式优先权排队中一旦分组开始传输就不可以打断。
  3. 循环（round robin queuing discipline）和加权公平排队（Weighted Fair Queuing,WFQ）。


## 4.3 网际协议：IPV4、寻址、IPV6及其他
### 4.3.1 IPV4数据报格式
- 版本：即IPV4或IPV6。
  首部长度：IP首部一般没有选项，共20字节。
  服务类型：实时、非实时、低时延、高吞吐量、可靠性等特殊标识。
  数据报长度：首部加数据总长度，单位字节。理论最大长度为2^16==65535bit，但是受到以太网帧长度限制，一般不超过1500字节。
  标识、标志、片偏移：与IP分片有关，IPV6不允许路由器对分组分片。
  协议：标识运输层协议。
  首部校验和：只对IP首部计算，TCP/UDP校验和对整个报文段计算。
  目的IP地址：源主机通过DNS查找来决定。
  选项：IPV6中去掉了。
  数据（有效载荷）：一般是运输层报文段，也可能是其他类型数据，如ICMP。
![IPV4](pic/IPV4.png)

### 4.3.2 IPV4数据报分片
- 链路层的MTU(Maximum Transmission Unit,MTU最大传送单元)限制了IP数据报长度。源到目的地路径上可能有多种链路层协议，可能需要分片。
- 某个数据报被分片后，最后一片的标志为0，其他分片的标志为1。

### 4.3.3 IPV4编址
- 主机或路由器与物理链路之间的边界叫接口（interface），一台主机或路由器可能有多个接口，每个接口都有一个全球唯一的IP地址。
- 点分十进制记法（dotted-decimal notation）：a.b.c.d，IPV4共4字节，用.分割字节。
- 子网（subnet）：相互连接的接口形成一个子网。
- 子网掩码（networkmask）：如a.b.c.d/x中的x指32bit中的最左侧x bit定义了子网地址，同一子网中的IP地址和子网地址的形式一样，即最左侧x bit相同。
![subnet](pic/subnet.png)

- 无类别域间路由选择（Classless Interdomain Routing,CIDR）：因特网的地址分配策略，将IP地址分为两部分，最高x bit构成了IP地址的网络部分，也称为前缀（prefix）。
>地址聚合（address aggregation）或路由聚合（route aggregation）或路由摘要（route summarization）：一个网络前缀下可能隐藏了多个网络。
最长前缀匹配：
![longest prefix matching rule](pic/longest%20prefix%20matching%20rule.png)

- IP广播地址：如255.255.255.255，报文会交付给同一子网中的所有主机。
- 动态主机配置协议（Dynamic Host Configuration,DHCP）：又称为即插即用（plug-and-play protocol）或零配置（zeroconf）协议，允许主机主动获取（被分配）一个IP地址。网络管理员可以通过DHCP配置主机每次连接网络时得到一个固定IP（通过MAC地址）或临时IP（每次连接网络时IP可能不同）。
- 最简情况下，每个子网都有一台DHCP服务器，或一个DHCP中继代理（通常是一台路由器），该代理知道DHCP服务器的地址。DHCP步骤：
  1. DHCP服务器发现：客户主机在UDP分组中向端口67发送DHCP发现报文（DHCP discover message），在网络层使用广播目的地址255.255.255.255，源IP地址0.0.0.0。
  2. DHCP提供：DHCP提供报文（DHCP offer message）使用广播目的地址255.255.255.255，内容包括：收到的DHCP发现报文的事务ID、向客户推荐的IP地址、网络掩码、IP地址租用期（address lease time，通常为几小时或几天）。
  3. DHCP请求：客户从一个或多个服务器提供中选一个，并返回DHCP请求报文（DHCP request message）。
  4. DHCP ACK：DHCP请求报文（DHCP ACK message）。
![DHCP](pic/DHCP.png)
>移动IP：允许节点在网络之间移动时使用单一永久的地址。

### 4.3.4 网络地址转换
- 网络地址转换（Network Address Translation,NAT）：管理IP地址的方法。
